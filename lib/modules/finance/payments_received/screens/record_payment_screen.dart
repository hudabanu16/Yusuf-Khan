...
class _RecordPaymentScreenState extends State<RecordPaymentScreen> {
    ...
    return Scaffold(
      ...
      body: ListenableBuilder(
          ...
          builder: (context, _) {
            ...
            // 🛠️ Dynamic Safe Guard: Ensure the current currency is ALWAYS in the dropdown list
            ...
            return Form(
              key: _ctrl.formKey,
              // 🛠️ Native live validation without crashing the build tree
              ...
              child: ListView(
                ...
                children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  if (_ctrl.availableCurrencies.length > 1)
                                    Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: Colors.orange.shade100, borderRadius: BorderRadius.circular(12)), child: const Text("⚠️ Multiple Currencies Found", style: TextStyle(color: Colors.orange, fontSize: 12, fontWeight: FontWeight.bold)))
                                ],
                              ),
                                ...
                                children: [
                                  Expanded(
                                    child: DropdownButtonFormField<String>(
                                      ...
                                      decoration: InputDecoration(
                                        ...
                                        filled: true, fillColor: isCurrencyLocked ? Colors.grey.shade100 : Colors.white,
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        TextFormField(
                                          ...
                                          decoration: InputDecoration(labelText: 'Exchange Rate (₹) *', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), filled: true, fillColor: Colors.white),
                                        ...
                                        if (_ctrl.selectedCurrency != 'INR')
                                          Padding(padding: const EdgeInsets.only(top: 6, left: 4), child: Text('1 ${_ctrl.selectedCurrency} = ₹ ${_ctrl.exchangeRateCtrl.text.isEmpty ? "0" : _ctrl.exchangeRateCtrl.text}', style: TextStyle(color: Colors.blueGrey.shade600, fontSize: 13, fontWeight: FontWeight.w600)))
                                      ],
                                    ),
                                  ),
                                  const SizedBox(width: 20),
                                  Expanded(
                                    child: TextFormField(
                                      ...
                                      decoration: InputDecoration(labelText: 'Amount Received (${_ctrl.selectedCurrency}) *', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), prefixText: '${_ctrl.selectedCurrency} ', filled: true, fillColor: Colors.white),
                          ...
                          children: [
                            Expanded(
                              child: DropdownButtonFormField<String>(
                                ...
                                decoration: InputDecoration(labelText: 'Payment Mode', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), filled: true, fillColor: Colors.white),
                                ...
                                onChanged: (v) { _ctrl.paymentMode = v!; _ctrl.notifyListeners(); },
                              ),
                            ),
                            const SizedBox(width: 20),
                            Expanded(
                              child: TextFormField(
                                ...
                                decoration: InputDecoration(labelText: 'Reference / Check No.', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), filled: true, fillColor: Colors.white),
                              ),
                            ),
                          ],
                        ),
                  ...
                  _buildSectionCard(
                    stepNumber: '2',
                    title: 'Allocate to Outstanding Invoices (${_ctrl.selectedCurrency})',
  ...
  Widget _buildCustomerSearchField() {
    return Autocomplete<Map<String, dynamic>>(
      ...
      fieldViewBuilder: (context, txtCtrl, focusNode, onFieldSubmitted) {
        if (_ctrl.selectedCustomerName.isNotEmpty && txtCtrl.text.isEmpty) txtCtrl.text = _ctrl.selectedCustomerName;
        bool hasSelection = _ctrl.selectedCustomerName.isNotEmpty;
        return TextFormField(
          controller: txtCtrl, focusNode: focusNode, readOnly: widget.customerName != null,
          decoration: InputDecoration(
            labelText: 'Search Customer *', hintText: 'Type customer name...',
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)),
            filled: true, fillColor: hasSelection ? Colors.blue.shade50 : Colors.white,
  ...
  Widget _buildDatePicker() {
    return InkWell(
      onTap: () async {
        final d = await showDatePicker(context: context, initialDate: _ctrl.paymentDate, firstDate: DateTime(2000), lastDate: DateTime.now());
        if (d != null) { _ctrl.paymentDate = d; _ctrl.notifyListeners(); }
      },
      child: InputDecorator(
        decoration: InputDecoration(labelText: 'Payment Date', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), filled: true, fillColor: Colors.white, suffixIcon: const Icon(Icons.calendar_today, size: 20, color: Colors.blueGrey)),
  ...
  Widget _buildAllocationState() {
    if (_ctrl.isLoadingInvoices) return const Padding(padding: EdgeInsets.all(40), child: Center(child: Column(children: [CircularProgressIndicator(), SizedBox(height: 16), Text("Fetching outstanding invoices...", style: TextStyle(color: Colors.grey))])));
    if (_ctrl.selectedCustomerId.isEmpty && _ctrl.selectedCustomerName.isEmpty) return _buildEmptyState(icon: Icons.person_search, title: "No Customer Selected", subtitle: "Search customer to view invoices.", color: Colors.blueGrey);

    if (_ctrl.filteredInvoices.isEmpty) return _buildEmptyState(
  ...
  Widget _buildEmptyState({required IconData icon, required String title, required String subtitle, required Color color}) {
    return Container(
      width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 48, horizontal: 24),
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 64, color: color.withOpacity(0.5)), const SizedBox(height: 16), Text(title, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: color)), const SizedBox(height: 8), Text(subtitle, style: const TextStyle(fontSize: 14, color: Colors.grey))]),
    );
  }
      ...
      child: SingleChildScrollView(
        ...
        child: ClipRRect(
          ...
          child: DataTable(
            ...
            columns: [
              const DataColumn(label: Text('Invoice No', style: TextStyle(fontWeight: FontWeight.w600))),
              const DataColumn(label: Text('Date', style: TextStyle(fontWeight: FontWeight.w600))),
              ...
              const DataColumn(label: Text('Allocate', style: TextStyle(fontWeight: FontWeight.w600))),
            ],
            rows: _ctrl.filteredInvoices.map((doc) {
              final data = doc.data() as Map<String, dynamic>;
              double total = (data['totals']?['grandTotal'] ?? 0.0).toDouble();
              double received = (data['amountReceived'] ?? 0.0).toDouble();
              double pending = data.containsKey('amountOutstanding') ? (data['amountOutstanding']).toDouble() : (total - received);
              DateTime dt = (data['invoiceDate'] as Timestamp).toDate();
              ...
              return DataRow(
                  ...
                  cells: [
                    ...
                    DataCell(Text(received.toStringAsFixed(2), style: const TextStyle(color: Colors.blueGrey))),
                    DataCell(Text(pending.toStringAsFixed(2), style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold))),
                    DataCell(
                        Padding(
                          ...
                          child: SizedBox(
                            width: 140,
                            child: TextFormField(
                              controller: _ctrl.allocationCtrls[doc.id],
                              keyboardType: const TextInputType.numberWithOptions(decimal: true), textAlign: TextAlign.right,
                              style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.green),
                              decoration: InputDecoration(hintText: '0.00', border: OutlineInputBorder(borderSide: BorderSide(color: errorMsg != null ? Colors.red : Colors.grey.shade400)), isDense: true, errorText: errorMsg),
                            ),
                          ),
                        )
                    ),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: Row(children: [CircleAvatar(radius: 14, backgroundColor: Colors.blue.shade100, child: Text(stepNumber, style: TextStyle(color: Colors.blue.shade800, fontWeight: FontWeight.bold, fontSize: 14))), const SizedBox(width: 12), Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E293B)))]),
        ),
        ...
        builder: (context, _) {
          ...
          return SafeArea(
            child: Container(
              ...
              decoration: BoxDecoration(color: Colors.white, border: Border(top: BorderSide(color: Colors.grey.shade300)), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 10, offset: const Offset(0, -5))]),
              child: Row(
                  ...
                  Column(
                    ...
                    children: [
                      ElevatedButton.icon(
                        ...
                        icon: _ctrl.isSaving ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : (isAdvanceOnly ? const Icon(Icons.account_balance_wallet) : const Icon(Icons.check_circle)),
                        label: Text(btnLabel, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      ),
                      if (disableReason.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 6), child: Text(disableReason, style: TextStyle(color: Colors.red.shade400, fontSize: 13, fontWeight: FontWeight.w600)))
                    ],
                  )
                ],
              ),
  ...
  Widget _buildSummaryStat(String label, double amount, String currency, Color color) {
    return Column(
      ...
      children: [Text(label, style: TextStyle(fontSize: 12, color: Colors.grey.shade600, fontWeight: FontWeight.w600)), const SizedBox(height: 4), Text('$currency ${amount.toStringAsFixed(2)}', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: color))],
    );
  }

  Widget _buildVerticalDivider() {
    return Container(height: 40, width: 1, color: Colors.grey.shade300, margin: const EdgeInsets.symmetric(horizontal: 24));
  }
}