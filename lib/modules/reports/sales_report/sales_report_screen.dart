...
class _SalesReportScreenContent extends StatefulWidget {
  ...
  @override
  State<_SalesReportScreenContent> createState() => _SalesReportScreenContentState();
}

class _SalesReportScreenContentState extends State<_SalesReportScreenContent> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        toolbarHeight: 0, // Crucial for preventing overflow when only using TabBar
  ...
  Widget _buildHeaderSection(BuildContext context, SalesReportController controller) {
                ...
                Wrap(
                  ...
                  children: [
                    OutlinedButton.icon(
                      ...
                      label: Text(_isFilterExpanded ? 'Close Filters' : 'Advanced Filter'),
                      style: OutlinedButton.styleFrom(
                        foregroundColor: _isFilterExpanded ? const Color(0xFF0F172A) : const Color(0xFF475569),
                        backgroundColor: _isFilterExpanded ? const Color(0xFFF1F5F9) : Colors.transparent,
                        side: BorderSide(color: _isFilterExpanded ? const Color(0xFFCBD5E1) : const Color(0xFFCBD5E1)),
  ...
  Widget _buildActiveFilterSummary(SalesReportController controller) {
    ...
    if (controller.startDate != null && controller.endDate != null) {
      summaryParts.add('${dateFormat.format(controller.startDate!)} - ${dateFormat.format(controller.endDate!)}');
    ...
    if (controller.selectedStatus != null) {
      summaryParts.add(controller.selectedStatus!.name[0].toUpperCase() + controller.selectedStatus!.name.substring(1));
    }
    ...
    final summaryText = summaryParts.isEmpty ? 'All Time' : summaryParts.join(' | ');
  ...
  Widget _buildQuickFilters(BuildContext context, SalesReportController controller) {
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          ...
          _QuickFilterChip(
            label: 'This Quarter',
            isSelected: _isThisQuarter(controller.startDate, controller.endDate),
            onTap: () {
  ...
  bool _isThisMonth(DateTime? start, DateTime? end) {
    ...
    return start.year == now.year && start.month == now.month && start.day == 1 &&
        end.year == now.year && end.month == now.month && end.day == DateTime(now.year, now.month + 1, 0).day;
  }
  ...
  bool _isLastMonth(DateTime? start, DateTime? end) {
    ...
    return start.year == lastMonth.year && start.month == lastMonth.month && start.day == 1 &&
        end.year == lastMonth.year && end.month == lastMonth.month && end.day == DateTime(now.year, now.month, 0).day;
  }
  ...
  bool _isThisQuarter(DateTime? start, DateTime? end) {
    ...
    return start.year == expectedStart.year && start.month == expectedStart.month && start.day == expectedStart.day &&
        end.year == expectedEnd.year && end.month == expectedEnd.month && end.day == expectedEnd.day;
  }
  ...
  bool _isThisYear(DateTime? start, DateTime? end) {
    ...
    return start.year == now.year && start.month == 1 && start.day == 1 &&
        end.year == now.year && end.month == 12 && end.day == 31;
  }

  Widget _buildFilterBar(BuildContext context, SalesReportController controller) {
    final dateFormat = DateFormat('MMM dd, yyyy');
    String dateText = 'Custom Dates';
    if (controller.startDate != null && controller.endDate != null) {
      dateText = '${dateFormat.format(controller.startDate!)} - ${dateFormat.format(controller.endDate!)}';
    } else if (controller.startDate != null) {
    ...
    final hasFilters = controller.startDate != null ||
    ...
    return Wrap(
      ...
      children: [
        ...
        _CompactDropdown<InvoiceStatus>(
          ...
          getName: (status) => status.name[0].toUpperCase() + status.name.substring(1),
    ...
    return Padding(
      ...
      child: Material(
        ...
        shape: RoundedRectangleBorder(
          ...
          side: BorderSide(color: isSelected ? themeColor.withOpacity(0.5) : Colors.transparent),
        ),
...
class _OverviewTab extends StatelessWidget {
      ...
      builder: (context, constraints) {
        ...
        return SingleChildScrollView(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              if (controller.summary.overdueAmount > 0)
                _buildOverdueAlert(),
              ...
              GridView(
                ...
                children: [
                  _KpiCard(title: 'Total Sales', amount: controller.summary.totalSales, icon: Icons.insights, color: const Color(0xFF3B82F6), trend: 0, onTap: () => controller.setStatus(null)),
                  _KpiCard(title: 'Collected', amount: controller.summary.collectedAmount, icon: Icons.account_balance_wallet_rounded, color: const Color(0xFF10B981), trend: 0, onTap: () => controller.setStatus(InvoiceStatus.paid)),
                  _KpiCard(title: 'Outstanding', amount: controller.summary.outstandingAmount, icon: Icons.hourglass_empty_rounded, color: const Color(0xFFF59E0B), trend: 0, onTap: () => controller.setStatus(InvoiceStatus.pending)),
                  _KpiCard(title: 'Overdue', amount: controller.summary.overdueAmount, icon: Icons.warning_amber_rounded, color: const Color(0xFFEF4444), trend: 0, onTap: () => controller.setStatus(InvoiceStatus.overdue)),
                ],
              ),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Overdue Payments Alert', style: TextStyle(color: Color(0xFF991B1B), fontWeight: FontWeight.w800, fontSize: 16)),
                const SizedBox(height: 4),
                Text(
          ...
          TextButton(
            ...
            child: const Text('View Overdue', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
          )
        ],
      ),
    );
    ...
    return _ChartCardContainer(
      ...
      child: Column(
        ...
        children: [
                  ...
                  PieChartData(
                    ...
                    sections: [
                      PieChartSectionData(
                        ...
                        color: total == 0 ? Colors.transparent : const Color(0xFF10B981),
                        title: '',
                        radius: 16,
                      ),
                      PieChartSectionData(
          ...
          const SizedBox(height: 32),
          _StatusCountRow(title: 'Total Invoices', count: total, color: const Color(0xFF94A3B8)),
          const SizedBox(height: 16),
          _StatusCountRow(title: 'Paid', count: paid, color: const Color(0xFF10B981)),
          const SizedBox(height: 16),
          _StatusCountRow(title: 'Pending', count: pending, color: const Color(0xFFF59E0B)),
          const SizedBox(height: 16),
          _StatusCountRow(title: 'Overdue', count: overdue, color: const Color(0xFFEF4444)),
        ],
      ),
    );
  }
  ...
  Widget _buildAgingAnalysisCard() {
    ...
    final currency = NumberFormat.compactCurrency(symbol: '₹', decimalDigits: 2);
    ...
    if (activeBuckets < 1 && maxVal == 0) {
      return const _ChartCardContainer(
        ...
        child: SizedBox(
          height: 320,
          child: Center(
            child: Text('No outstanding balances to analyze.', style: TextStyle(color: Color(0xFF94A3B8), fontSize: 14)),
          ),
        ),
      );
    }
      ...
      child: SizedBox(
        ...
        child: BarChart(
          BarChartData(
              ...
              bottomTitles: AxisTitles(
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (double value, TitleMeta meta) {
                    const style = TextStyle(color: Color(0xFF475569), fontWeight: FontWeight.w600, fontSize: 13);
                    String text = '';
                    switch(value.toInt()) {
                      case 0: text = '0-30 Days'; break;
                      case 1: text = '31-60 Days'; break;
                      case 2: text = '61-90 Days'; break;
                      case 3: text = '90+ Days'; break;
                    }
              ...
              leftTitles: AxisTitles(
                  sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        ...
                        return Text(currency.format(value), style: const TextStyle(color: Color(0xFF64748B), fontSize: 11, fontWeight: FontWeight.w500));
                      }
                  )
              ),
            ...
            barGroups: [
              BarChartGroupData(x: 0, barRods: [BarChartRodData(toY: buckets['0-30'] ?? 0, color: const Color(0xFF3B82F6), width: 40, borderRadius: BorderRadius.circular(6))]),
              BarChartGroupData(x: 1, barRods: [BarChartRodData(toY: buckets['31-60'] ?? 0, color: const Color(0xFFF59E0B), width: 40, borderRadius: BorderRadius.circular(6))]),
              BarChartGroupData(x: 2, barRods: [BarChartRodData(toY: buckets['61-90'] ?? 0, color: const Color(0xFFF97316), width: 40, borderRadius: BorderRadius.circular(6))]),
              BarChartGroupData(x: 3, barRods: [BarChartRodData(toY: buckets['90+'] ?? 0, color: const Color(0xFFEF4444), width: 40, borderRadius: BorderRadius.circular(6))]),
            ],
          ),
        ),
      ),
    ...
    if (data.length < 2) {
      return const _ChartCardContainer(
        ...
        child: SizedBox(
          height: 320,
          child: Center(
            child: Text('Not enough data to display trend.\nNeed at least 2 months of data.', textAlign: TextAlign.center, style: TextStyle(color: Color(0xFF94A3B8), fontSize: 14)),
          ),
        ),
      );
    }
                ...
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (value, meta) {
                    ...
                    return Padding(
                      ...
                      child: Text(data[value.toInt()].monthLabel, style: const TextStyle(color: Color(0xFF64748B), fontSize: 11, fontWeight: FontWeight.w600)),
                    );
                  },
                ),
              ),
  ...
  Widget _buildTopCustomersCard() {
    ...
    return _ChartCardContainer(
      ...
      child: top.isEmpty
          ? const SizedBox(
        height: 320,
        child: Center(child: Text('No customer data', style: TextStyle(color: Color(0xFF94A3B8), fontSize: 14))),
      )
          : SizedBox(
        height: 320,
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: index == 0 ? const Color(0xFFFEF08A).withOpacity(0.4) : const Color(0xFFEFF6FF),
                      shape: BoxShape.circle,
                    ),
                    child: Text(
                        '${index + 1}',
                        style: TextStyle(
                          color: index == 0 ? const Color(0xFFB45309) : const Color(0xFF2563EB),
...
class _StatusCountRow extends StatelessWidget {
  ...
  const _StatusCountRow({required this.title, required this.count, required this.color});
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Text('$count', style: const TextStyle(color: Color(0xFF0F172A), fontWeight: FontWeight.w800, fontSize: 15)),
      ],
    );
  }
}
          ...
          child: Container(
            ...
            child: SingleChildScrollView(
              ...
              child: DataTable(
                ...
                columns: const [
                  DataColumn(label: Text('Invoice No', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12))),
                  DataColumn(label: Text('Date', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12))),
                  DataColumn(label: Text('Customer Name', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12))),
                  DataColumn(label: Text('Type', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12))),
                  DataColumn(label: Text('Total', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12)), numeric: true),
                  DataColumn(label: Text('Paid', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12)), numeric: true),
                  DataColumn(label: Text('Balance', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12)), numeric: true),
                  DataColumn(label: Text('Status', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12))),
                ],
                rows: widget.controller.filteredInvoices.asMap().entries.map((entry) {
                  int idx = entry.key;
                  var inv = entry.value;
                  ...
                  return DataRow(
                    color: MaterialStateProperty.resolveWith<Color?>((states) {
                      if (states.contains(MaterialState.hovered) || isHovered) return const Color(0xFFF1F5F9);
                      return idx % 2 == 0 ? Colors.white : const Color(0xFFF8FAFC).withOpacity(0.5);
                    }),
                    cells: [
                      DataCell(
                          MouseRegion(
                              ...
                              child: Text(inv.invoiceNo, style: const TextStyle(fontWeight: FontWeight.w600, color: Color(0xFF0F172A), fontSize: 13))
                          )
                      ),
                      DataCell(Text(dateFmt.format(inv.date), style: const TextStyle(color: Color(0xFF475569), fontWeight: FontWeight.w500, fontSize: 13))),
                      DataCell(Text(inv.customerName, style: const TextStyle(color: Color(0xFF475569), fontWeight: FontWeight.w500, fontSize: 13))),
                      DataCell(Text(inv.type.name.toUpperCase(), style: const TextStyle(color: Color(0xFF94A3B8), fontSize: 11, fontWeight: FontWeight.w700))),
                      DataCell(Text(currency.format(inv.totalAmount), style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Color(0xFF1E293B)))),
                      DataCell(Text(currency.format(inv.paidAmount), style: const TextStyle(color: Color(0xFF059669), fontWeight: FontWeight.w600, fontSize: 13))),
                      DataCell(Text(currency.format(inv.balanceAmount), style: const TextStyle(color: Color(0xFFDC2626), fontWeight: FontWeight.w600, fontSize: 13))),
          ...
          child: Container(
            ...
            child: SingleChildScrollView(
              ...
              child: DataTable(
                ...
                columns: const [
                  DataColumn(label: Text('Customer Name', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12))),
                  DataColumn(label: Text('Total Sales', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12)), numeric: true),
                  DataColumn(label: Text('Received', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12)), numeric: true),
                  DataColumn(label: Text('Outstanding', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64748B), fontSize: 12)), numeric: true),
                ],
                    ...
                    cells: [
                      DataCell(
                          MouseRegion(
                              onEnter: (_) => setState(() => _hoveredIndex = idx),
                              onExit: (_) => setState(() => _hoveredIndex = null),
                              child: Text(cust.customerName, style: const TextStyle(fontWeight: FontWeight.w600, color: Color(0xFF0F172A), fontSize: 13))
                          )
                      ),
                      DataCell(Text(currency.format(cust.totalSales), style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Color(0xFF1E293B)))),
                      DataCell(Text(currency.format(cust.received), style: const TextStyle(color: Color(0xFF059669), fontWeight: FontWeight.w600, fontSize: 13))),
                      DataCell(Text(currency.format(cust.outstanding), style: const TextStyle(color: Color(0xFFD97706), fontWeight: FontWeight.w600, fontSize: 13))),
                    ],
                  );
                }).toList(),
              ),
...
class _KpiCardState extends State<_KpiCard> {
  ...
  @override
  Widget build(BuildContext context) {
    final format = widget.isCurrency
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<T?>(
          value: value,
          hint: Text(hint, style: const TextStyle(color: Color(0xFF64748B), fontSize: 14, fontWeight: FontWeight.w600)),
          icon: const Padding(
            padding: EdgeInsets.only(left: 12.0),
            child: Icon(Icons.unfold_more_rounded, size: 18, color: Color(0xFF94A3B8)),
          ),
          style: const TextStyle(color: Color(0xFF334155), fontSize: 14, fontWeight: FontWeight.w700),
          items: [
