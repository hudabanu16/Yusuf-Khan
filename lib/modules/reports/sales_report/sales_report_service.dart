...
class SalesReportService {
  ...
  Future<List<InvoiceData>> fetchInvoices({
    ...
    try {
      ...
      debugPrint(
        '[SalesReportService] Firestore Error (Company: $companyId): ${e.code} - ${e.message}',
      );
      throw Exception('Failed to load sales report data from the database.');
    } catch (e, stackTrace) {
      debugPrint(
        '[SalesReportService] Unknown Error (Company: $companyId): $e\n$stackTrace',
      );
      throw Exception(
        'An unexpected error occurred while processing sales data.',
      );
    }
  }
