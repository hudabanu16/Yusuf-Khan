...
class SalesReportController extends ChangeNotifier {
  ...
  void _calculateSummary() {
    ...
    _statusCounts = { InvoiceStatus.paid: 0, InvoiceStatus.pending: 0, InvoiceStatus.overdue: 0 };
    ...
    for (final invoice in _filteredInvoices) {
      ...
      if (invoice.balanceAmount > 0) {
        ...
        if (daysOld <= 30) {
          _agingBuckets['0-30'] = (_agingBuckets['0-30'] ?? 0) + invoice.balanceAmount;
        } else if (daysOld <= 60) {
          _agingBuckets['31-60'] = (_agingBuckets['31-60'] ?? 0) + invoice.balanceAmount;
        } else if (daysOld <= 90) {
          _agingBuckets['61-90'] = (_agingBuckets['61-90'] ?? 0) + invoice.balanceAmount;
        } else {
          _agingBuckets['90+'] = (_agingBuckets['90+'] ?? 0) + invoice.balanceAmount;
        }
      }
      ...
      customerMap[invoice.customerName] = CustomerSales(
        ...
        outstanding: _round((existing?.outstanding ?? 0.0) + invoice.balanceAmount),
      );
      ...
      monthlyMap[monthKey] = MonthlySalesData(
        ...
        collectedAmount: (existingMonth?.collectedAmount ?? 0.0) + invoice.paidAmount,
      );
    }
